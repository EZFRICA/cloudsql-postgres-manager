...
class ConnectionPool:
    ...
    def _create_connection(self, project_id: str, region: str, instance_name: str, 
    ...
    def get_connection(self, project_id: str, region: str, instance_name: str, 
                      database_name: str) -> Any:
        ...
        with self._lock:
            if self._created_connections < self.max_size + self.max_overflow:
                ...
                return self._create_connection(project_id, region, instance_name, database_name)
            else:
...
class ConnectionManager:
    ...
    def _get_or_create_pool(self, project_id: str, region: str, instance_name: str) -> ConnectionPool:
    ...
    def get_connection(
        ...
        try:
            logger.debug(f"Getting connection from pool for {instance_connection_name}/{database_name}")
            ...
            logger.debug(f"Connection obtained from pool for {instance_connection_name}/{database_name}")
            ...
            if conn and pool:
                try:
                    ...
                    logger.debug(f"Connection returned to pool for {instance_connection_name}/{database_name}")
