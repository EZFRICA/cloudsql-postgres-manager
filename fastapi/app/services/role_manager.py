...
class RoleManager:
    ...
    def _execute_sql_commands(
        self, connection, sql_commands: List[str], role_name: str
    ) -> bool:
        """
        ...
        try:
            ...
            for command in sql_commands:
                ...
                if not self.connection_manager.execute_sql_safely(cursor, command):
                    logger.error(
                        f"Failed to execute SQL command for {role_name}: {command}"
                    )
                    return False
            connection.commit()
            cursor.close()
    ...
    def _create_or_update_role(
        self, connection, role_def: RoleDefinition, force_update: bool = False
    ) -> Tuple[bool, str]:
        """
        ...
        if role_exists and force_update:
            logger.info(
                f"Role {role_def.name} exists, updating due to force_update=True"
            )
            action = "updated"
        else:
        ...
        success = self._execute_sql_commands(
            connection, role_def.sql_commands, role_def.name
        )
    ...
    def initialize_roles(
        ...
        try:
            logger.info(
                f"Starting role initialization for {project_id}/{instance_name}/{database_name}"
            )

            # Get existing registry
            registry = self.firestore_manager.get_role_registry(
            ...
            for plugin in self.plugin_registry.get_all_plugins().values():
                if hasattr(plugin, "get_role_definitions"):
                    plugin_roles = plugin.get_role_definitions(
            ...
            all_role_definitions = [
                rd
                for rd in all_role_definitions
            ...
            validation_result = RoleValidator.validate_multiple_roles(
                all_role_definitions
            )
            if not validation_result["valid"]:
                logger.warning(
                    f"Role validation found issues: {validation_result['summary']['errors']}"
                )
            ...
            with self.connection_manager.get_connection(
                ...
                for role_def in all_role_definitions:
                    try:
                        ...
                        role_validation = RoleValidator.validate_role_definition(
                            role_def
                        )
                        if not role_validation["valid"]:
                            logger.warning(
                                f"Role {role_def.name} failed validation: {role_validation['errors']}"
                            )
                            roles_skipped.append(role_def.name)
                            continue

                        # Create or update role
                        success, action = self._create_or_update_role(
                            connection, role_def, force_update
                        )
            ...
            if not firestore_success:
                logger.warning(
                    "Failed to save registry to Firestore, but roles were created successfully"
                )
            ...
            logger.info(
                f"Role initialization completed in {execution_time:.2f}s: {message}"
            )
    ...
    def get_role_status(
        ...
        """
        return self.firestore_manager.get_registry_status(
            project_id, instance_name, database_name
        )
