...
class PostgreSQLValidator:
    ...
    def validate_identifier(name: str, field_name: str = "identifier") -> str:
        ...
        if name.lower() in PostgreSQLValidator.RESERVED_KEYWORDS:
            raise ValueError(
                f"{field_name} cannot be a reserved PostgreSQL keyword: {name}"
            )
...
class RoleValidator:
        ...
        try:
            ...
            for command in role_def.sql_commands:
                ...
                for perm in cls.DANGEROUS_PERMISSIONS:
                    if perm in command_upper and not (
                        perm == "LOGIN" and "NOLOGIN" in command_upper
                    ):
                ...
                for pattern in cls.DANGEROUS_PATTERNS:
                    if pattern in command_upper:
                        ...
                        validation_result["security_checks"]["dangerous_patterns"] = (
                            True
                        )
                        validation_result["valid"] = False
            ...
            logger.info(
                f"Role validation completed for {role_def.name}: {'PASS' if validation_result['valid'] else 'FAIL'}"
            )
    ...
    @classmethod
    def validate_multiple_roles(
        cls, role_definitions: List[RoleDefinition]
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Multiple role validation completed: {overall_result['valid_roles']}/{overall_result['total_roles']} valid"
        )
