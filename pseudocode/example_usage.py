...
class CustomCloudSQLUserManager:
    ...
    def revoke_object_permissions_custom(
        self, cursor, username: str, schema_name: str
    ) -> bool:
        """
        ...
        try:
            ...
            if user_role == "admin":
                ...
                # Default behavior
                logger.warning(
                    f"Unknown role '{user_role}' for user {username}, using default revocation"
                )
                return self.revoke_all_object_permissions(cursor, username, schema_name)
    ...
    def revoke_all_object_permissions(
        self, cursor, username: str, schema_name: str
    ) -> bool:
        """
    ...
    def revoke_developer_permissions(
        self, cursor, username: str, schema_name: str
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Developer permissions revoked for {username} (routine access preserved)"
            )
            return success
    ...
    def revoke_viewer_permissions(
        ...
        try:
            ...
            logger.info(
                f"Viewer permissions revoked for {username} (sequence and routine access preserved)"
            )
            return success
